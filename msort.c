	...
	*/
	...
	void msort(Student * base, int nel, int (*compar)(const void *, const void *)) {
		...
		msort(base2, (nel - mid), compar);
		
		//3a. Merge the two arrays (use merge)

		Student * merge_base = merge(base1, mid, base2, (nel - mid), compar);
	...
	*/
	...
	Student * merge(Student * base1, int nel1, Student * base2, int nel2, int (*compar)(const void *, const void *)) {
		...
		int total = nel1 + nel2;
		Student * base = malloc(total * sizeof(Student));
		...
		int j = 0; // index for base2
		int k = 0; // index for main base
